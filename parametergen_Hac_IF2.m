function [lattice_par, E_vector, tao] = parametergen_Hac_IF2(m)
...
ai = composition(2*m,4)*a_AlSb + composition(2*m,3)*a_AlAs;

D = composition(2*m,4)*D_AlSb + composition(2*m,3)*D_AlAs;
...
Esa =   composition(2*m,4)*Esa_AlSb + composition(2*m,3)*Esa_AlAs; % 77K
Esc =   composition(2*m,4)*Esc_AlSb + composition(2*m,3)*Esc_AlAs;
Epa =    composition(2*m,4)*Epa_AlSb + composition(2*m,3)*Epa_AlAs;
Epc =    composition(2*m,4)*Epc_AlSb + composition(2*m,3)*Epc_AlAs;
Essa =    composition(2*m,4)*Essa_AlSb + composition(2*m,3)*Essa_AlAs;
Essc =    composition(2*m,4)*Essc_AlSb + composition(2*m,3)*Essc_AlAs;
Delta_a =    composition(2*m,4)*Delta_a_AlSb + composition(2*m,3)*Delta_a_AlAs;
Delta_c =    composition(2*m,4)*Delta_c_AlSb + composition(2*m,3)*Delta_c_AlAs;
Esasc =   composition(2*m,4)*Esasc_AlSb + composition(2*m,3)*Esasc_AlAs;
Esaxc =    composition(2*m,4)*Esaxc_AlSb + composition(2*m,3)*Esaxc_AlAs;
Exasc =    composition(2*m,4)*Exasc_AlSb + composition(2*m,3)*Exasc_AlAs;
Essaxc =    composition(2*m,4)*Essaxc_AlSb + composition(2*m,3)*Essaxc_AlAs;
Exassc =    composition(2*m,4)*Exassc_AlSb + composition(2*m,3)*Exassc_AlAs;
Exaxc =    composition(2*m,4)*Exaxc_AlSb + composition(2*m,3)*Exaxc_AlAs;
Exayc =    composition(2*m,4)*Exayc_AlSb + composition(2*m,3)*Exayc_AlAs;
