function [lattice_par, E_vector, tao] = parametergen_Hca_InAs(m)
...
ai = composition(2*m-1,4)*a_InSb + composition(2*m-1,3)*a_InAs;

D = composition(2*m-1,4)*D_InSb + composition(2*m-1,3)*D_InAs;

exx = a_para/ai - 1; eyy = exx; ezz = -D * exx;
...
Esa =   composition(2*m-1,4)*Esa_InSb + composition(2*m-1,3)*Esa_InAs; % 77K
Esc =   composition(2*m-1,4)*Esc_InSb + composition(2*m-1,3)*Esc_InAs;
Epa =    composition(2*m-1,4)*Epa_InSb + composition(2*m-1,3)*Epa_InAs;
Epc =    composition(2*m-1,4)*Epc_InSb + composition(2*m-1,3)*Epc_InAs;
Essa =    composition(2*m-1,4)*Essa_InSb + composition(2*m-1,3)*Essa_InAs;
Essc =    composition(2*m-1,4)*Essc_InSb + composition(2*m-1,3)*Essc_InAs;
Delta_a =    composition(2*m-1,4)*Delta_a_InSb + composition(2*m-1,3)*Delta_a_InAs;
Delta_c =    composition(2*m-1,4)*Delta_c_InSb + composition(2*m-1,3)*Delta_c_InAs;
Esasc =   composition(2*m-1,4)*Esasc_InSb + composition(2*m-1,3)*Esasc_InAs;
Esaxc =    composition(2*m-1,4)*Esaxc_InSb + composition(2*m-1,3)*Esaxc_InAs;
Exasc =    composition(2*m-1,4)*Exasc_InSb + composition(2*m-1,3)*Exasc_InAs;
Essaxc =    composition(2*m-1,4)*Essaxc_InSb + composition(2*m-1,3)*Essaxc_InAs;
Exassc =    composition(2*m-1,4)*Exassc_InSb + composition(2*m-1,3)*Exassc_InAs;
Exaxc =    composition(2*m-1,4)*Exaxc_InSb + composition(2*m-1,3)*Exaxc_InAs;
Exayc =    composition(2*m-1,4)*Exayc_InSb + composition(2*m-1,3)*Exayc_InAs;
